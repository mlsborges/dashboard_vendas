...
with aba3:
    qtd_vendendores = st.number_input('Quantidad de Vendedores', 2, 10, 5)
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.metric('Receita', formata_numero(dados['Preço'].sum(), 'R$'))
        fig_receita_vendedores = px.bar(vendedores[['sum']].sort_values('sum', ascending = True).head(qtd_vendendores),
                                    x = 'sum',
                                    y = vendedores[['sum']].sort_values('sum', ascending = True).head(qtd_vendendores).index,
                                    text_auto = True,
                                    title = f'Top {qtd_vendendores} vendedores (receita)')
        st.plotly_chart(fig_receita_vendedores)
    with coluna2:
        st.metric('Quantidade de Vendas', formata_numero(dados.shape[0]))
        fig_venda_vendedores = px.bar(vendedores[['count']].sort_values('count', ascending = True).head(qtd_vendendores),
                                    x = 'count',
                                    y = vendedores[['count']].sort_values('count', ascending = True).head(qtd_vendendores).index,
                                    text_auto = True,
                                    title = f'Top {qtd_vendendores} vendedores (vendas)')
